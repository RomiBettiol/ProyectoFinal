...
export default function MiPerfil({ navigation }) {
  ...
  useEffect(() => {
    axios
      .get(
        ` https://e860-181-91-230-36.ngrok-free.app/publications/publication/ByUser`,
        {
          headers: {
            "auth-token": token,
          },
  ...
  const handleUpdatePassword = () => {
    if (currentPassword === "" || newPassword === "" || repeatPassword === "") {
      ...
      axios
        .put(
          ` https://e860-181-91-230-36.ngrok-free.app/security/user/${idUser}`,
          updatedUserData,
          {
            headers: {
              "auth-token": token,
  ...
  const handleDeletePublication = (publicationToDelete) => {
    ...
    if (publicationToDelete) {
      ...
      axios
        .delete(
          ` https://e860-181-91-230-36.ngrok-free.app/publications/publication/${idPublicationToDelete}?modelType=${modalType}`,
          {
            headers: {
              "auth-token": token,
            },
  ...
  const handleLogout = async () => {
    try {
      const response = await fetch(
        ` https://e860-181-91-230-36.ngrok-free.app/security/auth/logout`,
        {
          method: "POST",
          headers: {
