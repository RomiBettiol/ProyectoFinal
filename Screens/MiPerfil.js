...
export default function MiPerfil({ navigation }) {
  ...
  const [confirmationModalService, setConfirmationModalService] =
    useState(false);
  ...
  useEffect(() => {
    ...
    const publicationsRequest = axios.get(
      `https://37e1-186-12-32-189.ngrok-free.app/publications/publication/ByUser`,
      {
        headers: {
          "auth-token": token,
        },
    ...
    const servicesRequest = axios.get(
      `https://37e1-186-12-32-189.ngrok-free.app/services/service/ByUser`,
      {
        headers: {
          "auth-token": token,
        },
  ...
  const handleUpdatePassword = async () => {
    if (currentPassword === "" || newPassword === "" || repeatPassword === "") {
      ...
      axios
        .put(
          `https://37e1-186-12-32-189.ngrok-free.app/security/user/${idUser}`,
          updatedUserData,
          {
            headers: {
              "auth-token": token,
  ...
  const handleDeletePublication = (publicationToDelete) => {
    ...
    if (publicationToDelete) {
      ...
      axios
        .delete(
          `https://37e1-186-12-32-189.ngrok-free.app/publications/publication/${idPublicationToDelete}?modelType=${modalType}`,
          {
            headers: {
              "auth-token": token,
            },
  ...
  const handleConfirmDelete = () => {
    ...
    axios
      .delete(
        `https://37e1-186-12-32-189.ngrok-free.app/services/service/${idServicio}`,
        {
          headers: {
          ...
          <View style={styles.modalContainer}>
            <View style={styles.modalContent}>
              <TouchableOpacity
                style={styles.opcionesModal}
                onPress={() => {
                  navigation.navigate("EditarPublicacionServicio", {
                    servicio,
                    token,
                  });
