    ...
    try {
      ...
      if (state === "ACTIVO") {
        response = await axios.delete(
          `https://romibettiol.loca.lt/security/${type}/${id}`,
          config
        );
      } else {
        response = await axios.post(
          `https://romibettiol.loca.lt/security/${type}/active/${id}`,
          {},
          config
        );
      }
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      if (type === "role") {
        ...
        response = await axios.post(
          `https://romibettiol.loca.lt/security/${type}/`,
          data,
          config
        );
      } else {
  ...
  const managePermission = async (idPermission, tokenClaim) => {
    ...
    try {
      ...
      if (permissionRole.includes(tokenClaim)) {
        response = await axios.delete(
          `https://romibettiol.loca.lt/security/role/take/${id}/${idPermission}`,
          config
        );
      } else {
        response = await axios.post(
          `https://romibettiol.loca.lt/security/role/add/${id}`,
          data,
          config
        );
      }
