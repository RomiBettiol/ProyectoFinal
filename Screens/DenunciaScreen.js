...
export default function DenunciaScreen({ navigation }) {
  ...
  function loadDenuncias(token) {
    ...
    axios
      .get("  https://2f6b-181-91-230-36.ngrok-free.app/security/complaint/", {
        headers: {
          "auth-token": token,
        },
      })
  ...
  function handleRechazar() {
    if (denunciaToReject) {
      ...
      axios
        .delete(
          `  https://2f6b-181-91-230-36.ngrok-free.app/security/complaint/${denunciaToReject}`,
          { headers }
        )
        .then((response) => {
  ...
  function handleBloquear(idComplaint, token) {
    ...
    axios
      .post(
        `  https://2f6b-181-91-230-36.ngrok-free.app//security/complaint/execute/${data.validate}/${idComplaint}`,
        null,
        {
