...
import { StyleSheet,ScrollView, Text,TextInput, TouchableOpacity, View,KeyboardAvoidingView,Pressable } from 'react-native';
...
export default function DocumentosRegistrarseEmpresaScreen({ route }) {
  ...
  console.log('despues de navegar:',checkBoxData, email, isValidEmail, nombre, usuario, contrasena, images, domicilio, nroTelefono,fechaNacimiento,cuitCuil)
  ...
  console.log('despues de navegar tiendasMascotas:',checkBoxData.tiendasMascotas)
  const [pdf1, setPdf1] = useState(null);
  const [pdf2, setPdf2] = useState(null);
  const [pdf3, setPdf3] = useState(null);
  const [nombrePdf1, setNombrePdf1] = useState('ADJUNTAR HABILITACION MUNICIPAL');
  ...
  }, [checkBoxData.servicioSalud, checkBoxData.tiendasMascotas, checkBoxData.refugioMascotas]);
  ...
  const sendDocData = async (docLink) => {
    ...
    const response = await axios.post('https://8396-191-82-3-33.ngrok-free.app/security/user/register', data, config);
