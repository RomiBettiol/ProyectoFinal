...
import { View, Text, ScrollView, StyleSheet, TouchableOpacity, Image, Modal, Dimensions, TextInput } from 'react-native';
...
export default function Chats({navigation, chatId, idUserRecep, nombre, image}){
    ...
    const [imagePublicacion, setImagePublicacion] = useState(data.imagenPublicacion || "");
    ...
    const fetchArchivar = async () => {
      try {
          ...
          const response = await axios.post(`https://8396-191-82-3-33.ngrok-free.app/chats/chat/archive/${idChat}/true`,null, {
              headers: {
                  'auth-token': token
              }
          });
  ...
  const fetchCerrarSearch = async () => {
    ...
    try {
        ...
        const response = await axios.post(`https://8396-191-82-3-33.ngrok-free.app/publications/publication/solve/${idPublicacion}?modelType=Search`,null,{
            headers: {
                'auth-token': token
            }
        });
...
const fetchCerrarAdoption = async () => {
  ...
  try {
      console.log("Estoy adentro del try cerrar, token:", token);                             
      const response = await axios.post(`https://8396-191-82-3-33.ngrok-free.app/publications/publication/solve/${idPublicacion}?modelType=Adoption`,null,{
          headers: {
              'auth-token': token
          }
      });
  ...
  useEffect(() => {
    ...
    const fetchSMS = async () => {
      try {
          ...
          const response = await axios.get(`https://8396-191-82-3-33.ngrok-free.app/chats/message/${idChat}`, {
              headers: {
                  'auth-token': token
              }
          });
    ...
    const fetchEnviar= async () => {
      ...
      try {
        ...
        const response = await axios.post(`https://8396-191-82-3-33.ngrok-free.app/chats/message/${idChat}`,
        {
            headers: {
                'auth-token': token
            },
  ...
  const handleEnviarSMS = async () => {
    try {
      const response = await axios.post(
        `https://8396-191-82-3-33.ngrok-free.app/chats/message/${idChat}`,
        {
          content: nuevoSMS,
        },
        {
                    ...
                    >
                      <View style={styles.modalContainer}>
                        <View style={styles.modalContent}>
                          <Text style={styles.modalText}>¿Desea archivar el chat?</Text>
                       ...
                       <View style={styles.modalContent}>
                        ...
                        {referenceType[0] === "SEARCH" ? (
                          <View style={alignItems='center'}>
                            <Text style={styles.modalText}>¡Mascota encontrada!</Text>
                            <View style={styles.modalButtons}>
                              <TouchableOpacity
                                style={styles.modalButton}
                            ...
                            <View>
                              <Text style={styles.modalText}>No hay acciones disponibles</Text>
                              <View style={styles.modalButtons}>
                                <TouchableOpacity
                                  style={styles.modalButton}
                                  onPress={handleDeleteCancelled}
            ...
            >
              <View style={styles.contenedor}>
                {SMS.map((mensaje, index) => (
                  ...
                  {index === 0 || formatDate(SMS[index - 1].createdAt) !== formatDate(mensaje.createdAt) ? (
                    <View style={styles.dateContainer}>
                      <Text style={styles.dateText}>{formatDate(mensaje.createdAt)}</Text>
                    </View>
                  ) : null}
                    ...
                    <View >
                      <View style={styles.fila}>
                        <Image
                          ...
                          />
                          <Text style={[
                                  styles.userItemTitle, styles.userItemContent,
                                  {
                                      alignSelf: 'flex-start' ,
            ...
            >
              <View style={styles.modalContainer}>
                <View style={[styles.modalContent,
                  ...
                  <Text style={styles.modalText}>¡Felicitaciones! Nos alegra que esta busqueda haya tenido éxito.</Text>
                </View>
              </View>
            </Modal>
