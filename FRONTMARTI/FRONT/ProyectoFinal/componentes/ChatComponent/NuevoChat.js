...
export default function NuevoChat({navigation}) {
    ...
    const handleNewChat = async (selectedUser) => {
        ...
        console.log("idUserautor: ", idAutor, "idUser seleccionado: ", idSelectedUser)
        try {
          // Realizar una solicitud POST al servidor para crear un nuevo chat
          const response = await axios.post(`https://8396-191-82-3-33.ngrok-free.app/chats/chat/${idSelectedUser}`,null,{
              headers: {
                  'auth-token': token
                }
            });
    ...
    const fetchUsers = async () => {
        ...
        try {
          ...
          const response = await axios.get('https://8396-191-82-3-33.ngrok-free.app/security/user/all',{
            headers: {
                'auth-token': token
              }
          });
