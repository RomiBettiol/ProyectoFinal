...
function contarMensajesNoLeidos(chat) {
  ...
  useEffect(() => {
    axios
      .get('https://8396-191-82-3-33.ngrok-free.app/chats/chat/true', {
        headers: {
          'auth-token': token,
        },
      })
  ...
  const fetchDesArchivar = async () => {
    ...
    try {
        ...
        const response = await axios.post(`https://8396-191-82-3-33.ngrok-free.app/chats/chat/archive/${selectedIdChat}/false`,null, {
            headers: {
                'auth-token': token
            }
        });
