...
const ModalAgregarRaza = ({ isVisible, onClose, onAdd, petTypes, onSuccess, onError }) => {
  ...
  const handleAddRaza =async () => {
    ...
    await axios
      .post('http://romibettiol.loca.lt/parameters/petBreed', newBreed)
      .then((response) => {
        ...
        <View style={styles.modalContent}>
          ...
          >
            <Text style={styles.regionDropdownText}>
              {selectedPetTypeId ? petTypes.find(type => type.idPetType === selectedPetTypeId).petTypeName : 'Seleccionar Tipo de Animal'}
            </Text>
          </TouchableOpacity>
          ...
          <View style={[styles.botonesDecidir, {flexDirection: 'row'}]}>
            <TouchableOpacity style={styles.botonesEditar} onPress={handleAddRaza}>
