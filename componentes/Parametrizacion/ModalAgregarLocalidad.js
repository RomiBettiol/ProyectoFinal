...
const ModalAgregarLocalidad = ({ isVisible, onClose, onAdd, regions, onSuccess, onError }) => {
  ...
  const handleAddLocalidad = () => {
    ...
    axios
      .post('http://romibettiol.loca.lt/parameters/locality', newLocalidad)
      .then((response) => {
        onAdd(newLocalidad);
        setLocalityName('');
        setSelectedRegionId('');
        ...
        <View style={styles.modalContent}>
          ...
          />
            ...
            <Text style={styles.regionDropdownText}>
                {selectedRegionId ? regions.find(region => region.idRegion === selectedRegionId).regionName : 'Seleccionar Región'}
            </Text>
            </TouchableOpacity>
            {showRegionList && (
          ...
          <View style={[styles.botonesDecidir, {flexDirection: 'row'}]}>
            <TouchableOpacity style={styles.botonesEditar} onPress={handleAddLocalidad}>
